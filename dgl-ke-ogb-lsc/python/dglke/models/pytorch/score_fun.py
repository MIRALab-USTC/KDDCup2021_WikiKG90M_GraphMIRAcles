...
class DistMultScore(nn.Module):
    ...
    def edge_func(self, edges):
        ...
        if self.args.reg == 'None' or self.args.reg == 'N3':
            return {'score': th.sum(score, -1)}

        elif self.args.reg == 'N3-N':
            norm = th.abs(head) ** 3 + th.abs(rel) ** 3 + th.abs(tail) ** 3
            return {'score': th.sum(score, -1), 'norm': norm}

        elif self.args.reg == 'DURA':
            norm = self.args.w1 * th.sum(tail ** 2 + head ** 2, -1) + self.args.w2 * th.sum(head ** 2 * rel ** 2 + tail ** 2 * rel ** 2, -1)
            return {'score': th.sum(score, -1), 'norm': norm}
...
class ComplExScore(nn.Module):
    ...
    def edge_func(self, edges):
        ...
        if self.args.reg == 'None' or self.args.reg == 'N3':
            ...
            h = th.sqrt(real_head ** 2 + img_head ** 2)
            r = th.sqrt(real_rel ** 2 + img_rel ** 2)
            t = th.sqrt(real_tail ** 2 + img_tail ** 2)

            norm = th.abs(h) ** 3 + th.abs(r) ** 3 + th.abs(t) ** 3
            return {'score': th.sum(score, -1), 'norm': norm}
            ...
            norm = th.sum(t ** 2 + h ** 2, -1) + th.sum(h ** 2 * r ** 2 + t ** 2 * r ** 2, -1)
            return {'score': th.sum(score, -1), 'norm': norm}
...
class SimplEScore(nn.Module):
    ...
    def edge_func(self, edges):
        ...
        score = 1 / 2 * (forward_score + backward_score).sum(-1)
