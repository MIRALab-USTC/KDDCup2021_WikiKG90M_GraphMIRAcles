...
class ArgParser(CommonArgParser):
    def __init__(self):
        ...
        self.add_argument('--encoder_model_name', type=str, default='shallow',
                          help='shallow or roberta or concat or concat_simple')
        ...
        self.add_argument('--seed', type=int, default=None,
                          help='random seed')
        self.add_argument('--instance_id', type=str, default='default',
                          help='The id of the instance')

        self.add_argument('--reg', choices=['None', 'N3', 'N3-N', 'DURA'], default='N3')
        # for DURA
        self.add_argument('--w1', type=float, default=1.0)
        self.add_argument('--w2', type=float, default=1.0)

        self.add_argument('--inverse_relation', action='store_true')
        self.add_argument('--weight_decay', type=float, default=0.0)

        self.add_argument('--n_split', type=int, default=1)
...
def prepare_save_path(args):
    ...
    folder = '{}_{}_{}_d_{}_g_{}_{}'.format(args.model_name, args.dataset, args.encoder_model_name, args.hidden_dim,
                                            args.gamma, args.instance_id)
    ...
    with open(os.path.join(args.save_path, 'run_cmd.txt'), 'w') as f:
        f.write(run_cmd)

    # copy the code to checkpoint directory
    cwd = os.getcwd()
    code_dir = os.path.join(cwd, 'dgl-ke-ogb-lsc')
    backup_dir = os.path.join(args.save_path, 'code_backup')
    os.makedirs(backup_dir)

    subprocess.run(["cp", "-r", code_dir, backup_dir])
...
def main():
    ...
    # if True:
    #     # if args.batch_size <= args.neg_sample_size:
    #     neg_per_sample = args.num_neg_chunk * args.neg_sample_size // args.batch_size
    #     neg_chunk_size = args.batch_size // args.num_neg_chunk
    # else:
    #     neg_per_sample = 1
    ...
    if args.num_proc > 1:
        ...
        for i in range(args.num_proc):
            ...
            train_samplers.append(NewBidirectionalOneShotIterator(args, train_sampler_head, train_sampler_tail,
                                                                  neg_chunk_size, args.neg_sample_size,
                                                                  True, dataset.n_entities,
                                                                  args.has_edge_importance))
            print("Training sampler for proc %d created, it takes %s seconds" % (i, time.time() - t1))

        train_sampler = NewBidirectionalOneShotIterator(args, train_sampler_head, train_sampler_tail,
    ...
    if args.encoder_model_name not in ['shallow']:
        ...
        if args.inverse_relation:
            model.relation_feat.emb = np.concatenate((dataset.relation_feat, dataset.relation_feat), axis=0)
            # model.relation_feat.emb = np.concatenate((dataset.relation_feat, -dataset.relation_feat), axis=0)
        else:
            model.relation_feat.emb = dataset.relation_feat
    print("Model created, it takes %s seconds" % (time.time() - t1))

    # print("loading feat from disk to cache ...")
    # t1 = time.time()
    # torch.from_numpy(model.entity_feat.emb[:]).to(torch.float)
    # torch.from_numpy(model.relation_feat.emb[:]).to(torch.float)
    # print("feat loaded, it takes %s seconds" % (time.time()-t1))
