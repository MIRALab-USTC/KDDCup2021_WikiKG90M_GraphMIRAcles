...
def ConstructGraph(edges, n_entities, n_relations, args):
...
class TrainDataset(object):
    ...
    def __init__(self, dataset, args, ranks=64, has_importance=False):
        if args.inverse_relation:
            tmp = np.copy(dataset.train)
            tmp[[0, 2]] = tmp[[2, 0]]
            tmp[1] += dataset.n_relations
            dataset.train = np.concatenate((dataset.train, tmp), axis=1)
        ...
        print("Edge partition finished!")
        print("Constructing Graph...")
        self.g = ConstructGraph(triples, dataset.n_entities, dataset.n_relations, args)

    def create_sampler(self, batch_size, neg_sample_size=0, neg_chunk_size=0, mode='head', num_workers=32,
...
class NewBidirectionalOneShotIterator:
    ...
    num_nodes : int
        Total number of nodes in the whole graph.
    """
    def __init__(self, args, dataloader_head, dataloader_tail, neg_chunk_size, neg_sample_size,
